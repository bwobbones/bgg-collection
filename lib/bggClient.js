...
export async function fetchBGGCollection(username, options = {}) {
  ...
  if (verbose) {
    console.log(
      chalk.cyan.bold(`[STEP 1/4] 📦 Requesting collection XML for user "${username}"...`)
    );
  }
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (isQueued) {
        ...
        if (verbose) {
          console.log(chalk.yellow(`  ⏳ ${queueMsg}`));
        }
        if (onProgress) {
          onProgress({
            step: "queue",
      ...
      if (verbose) {
        console.log(chalk.green(`  ✔ Collection XML successfully received.`));
      }
      ...
      if (verbose) {
        console.log(chalk.yellow(`  ⚠️ Attempt ${attempt} failed (${err.message}). Retrying...`));
      }
      await new Promise((resolve) => setTimeout(resolve, retryDelay));
    }
  }
...
export async function fetchBGGThings(ids, options = {}) {
  ...
  if (verbose) {
    console.log(
      chalk.cyan.bold(
        `[STEP 2/4] 🔍 Fetching "Best At" poll data & item types for ${ids.length} games (${chunks.length} batches)...`
      )
    );
  }
    ...
    try {
      ...
      if (
        ...
        if (retryCount < 5) {
          ...
          if (verbose) {
            console.log(chalk.yellow(`  ⏳ ${rateMsg}`));
          }
          if (onProgress) {
            onProgress({
              step: "ratelimit",
  ...
  for (let i = 0; i < chunks.length; i += batchSize) {
    ...
    const batchNum = String(Math.ceil(fetchedCount / maxPerChunk)).padStart(2, "0");
    const totalBatches = String(chunks.length).padStart(2, "0");
    const progressMsg = `Loaded batch ${batchNum}/${totalBatches} (${fetchedCount}/${ids.length} games - ${pct}%)`;

    if (verbose) {
      console.log(chalk.slate ? chalk.slate(`  ├─ ${progressMsg}`) : `  ├─ ${progressMsg}`);
    }
    ...
    if (onProgress) {
      onProgress({
        ...
        total: ids.length,
        percentage: pct,
        message: `Fetching game details: ${fetchedCount} of ${ids.length} games loaded (${pct}%)...`,
      });
    }
  ...
  if (verbose) {
    console.log(chalk.green(`  ✔ All ${ids.length} game details & poll data fetched.`));
  }
