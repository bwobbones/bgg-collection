  ...
  const items = rawItems.map((item) => {
    ...
    if (statsNode) {
      ...
      if (ratingNode) {
        ...
        const usersRatedNode = ratingNode.usersrated;
        if (usersRatedNode) {
          const rawUsersRated = getAttr(usersRatedNode, "value");
          if (rawUsersRated && rawUsersRated !== "N/A") {
            usersRated = parseInt(rawUsersRated, 10);
          }
        }
