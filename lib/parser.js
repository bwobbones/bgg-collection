    ...
    for (const ps of pollSummaries) {
      if (getAttr(ps, "name") === "suggested_numplayers" || !getAttr(ps, "name")) {
        ...
        if (bestRes) {
          ...
          if (val) {
            // Clean "Best with 2 players" -> "2 players"
...
export function parseBGGCollectionXML(xmlString) {
  ...
  const items = rawItems.map((item) => {
    ...
    return {
      ...
      maxPlayers,
      playingTime,
      bestAt: null, // enriched via thing API
      recommendedAt: null,
      comment,
      image,
      thumbnail,
...
/**
 * Parse thing XML string(s) to map game IDs to "best at" and "recommended at" player counts
 *
 * @param {Array<string>} thingXmlList - Array of XML strings from BGG thing API
 * @returns {Map<number, { bestAt: string|null, recommendedAt: string|null }>}
 */
  ...
  for (const xmlString of thingXmlList) {
    ...
    try {
      ...
      for (const item of rawItems) {
        ...
        detailsMap.set(id, { bestAt, recommendedAt });
      }
    } catch (e) {
