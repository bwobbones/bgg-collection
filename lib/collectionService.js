...
import { getExclusionsForUser } from "./exclusions.js";
...
export async function getProcessedCollection(params = {}) {
  ...
  const parsedPlayerCounts = playerCounts
  ...
  if (verbose) {
    ...
    console.log(
      chalk.gray(
        `   Player Counts Filter: [${parsedPlayerCounts ? parsedPlayerCounts.join(", ") : "ALL"}] | Include Expansions: ${includeExpansions}`
      )
    );
    console.log(chalk.bold.magenta("🎲 ═════════════════════════════════════════════════════════════════🎲\n"));
  }
  ...
  if (!noBest) {
    ...
    for (const item of filteredItems) {
      ...
      const commBest = details?.bestAt && details.bestAt !== "(Undetermined)" ? details.bestAt : null;
  ...
  // 4. Base Filter 2: User-keyed Exclusion List (unless includeExclusions is true)
  const userExclusions = getExclusionsForUser(username);
  const excludedSet = new Set(
    userExclusions.map((e) => e.trim().toLowerCase())
  );
  const excludedCount = filteredItems.filter((item) =>
    excludedSet.has(item.name.trim().toLowerCase())
  ).length;
  ...
  // Calculate Played metrics: games with at least one logged play
  const playedCount = filteredItems.filter((item) => (item.numPlays || 0) > 0).length;
  ...
  if (cleanMode === "shit") {
    filteredItems = filteredItems.filter(
      (item) =>
        ...
        (parsedPlayerCounts ? matchesPlayerCounts(item, parsedPlayerCounts) : true)
    );
    ...
    // General mode: filter by selected player counts if explicitly provided
    if (parsedPlayerCounts) {
      filteredItems = filteredItems.filter((item) =>
  ...
  if (verbose) {
    ...
    console.log(
      `  └─ Played Metrics (at least 1 play): ${playedCount} / ${totalEligibleCount} (${playedPercentage}%) — ${unplayedCount} unplayed`
    );
    console.log(
      chalk.bold.magenta("🎲 ═════════════════════════════════════════════════════════════════🎲\n")
    );
  ...
  return {
    ...
    totalItems,
    totalEligibleCount,
    userExclusionsCount: userExclusions.length,
