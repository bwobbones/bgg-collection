...
export async function getProcessedCollection(params = {}) {
  ...
  // 2. Base Filter: Strictly OWNED games (unless all or specific status flags set)
  let filteredItems = [...items];
  if (!all && (own || (!played && !rated && !wishlist))) {
    filteredItems = filteredItems.filter((item) => item.status.own);
  }

  // 3. Base Filter: EXCLUSION_LIST
  ...
  // 4. Base Filter: Exclude Expansions (unless includeExpansions or subtype expansion requested)
  ...
  // Helper to enrich items with "Best At" poll data
  ...
  // Enrich with thing details if running preset mode or bestAt filter
  const cleanMode = mode ? mode.trim().toLowerCase() : null;
  if ((cleanMode || bestAt) && !noBest) {
  ...
  // Enrich final items if not already enriched
  if (!cleanMode && !bestAt && !noBest) {
    await enrichWithThings(filteredItems);
  }
