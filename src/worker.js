 ...
 * Handles /api/collection, /api/collection/stream, /api/exclusions, /api/plays/pages, /api/discord/spin, /api/test
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 6. Play history pages: GET /api/plays/pages
    // Returns a small bundle of BGG play-history pages (newest first). The browser
    // pages through bundles and assembles the timeline, which keeps every Worker
    // invocation well under the 50-subrequest limit. Pages are cached in KV.
    if (url.pathname === "/api/plays/pages") {
      const token = env?.BGG_TOKEN || globalThis?.BGG_TOKEN || process?.env?.BGG_TOKEN || null;
      const username = (url.searchParams.get("username") || env?.BGG_USERNAME || "").trim();
      const from = parseInt(url.searchParams.get("from") || "1", 10);
      const countParam = parseInt(url.searchParams.get("count") || String(DEFAULT_PAGES_PER_REQUEST), 10);
      ...
      try {
        const data = await fetchPlaysPages({
          ...
          count,
          cache: env?.BGG_EXCLUSIONS_KV || null,
          forceRefresh,
        });
        ...
        return new Response(
          JSON.stringify({ success: false, error: err.message || "Failed to load the play history" }),
